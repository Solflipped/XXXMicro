...
class KOPreprocessor:
    """KO abundance preprocessor using legacy zero-imputation + log transform."""

    def __init__(
        self,
        file_path,
        outlier_method="iqr",
        root_output="/root/XXXMicro/Data/EW-T2D-5-13/ko_preprocessed",
    ):
    ...
    def load_raw_data(self):
        """Load the raw KO abundance table."""
        print("=" * 70)
        print("步骤1: 加载 KO 原始数据")
        print("=" * 70)
    ...
    def preprocess_data(self):
        """Preprocess KO data: remove all-zero features + zero imputation + log transform."""
        print("\n" + "=" * 70)
        print("步骤2: KO 数据预处理")
        print("=" * 70)
        ...
        features = features.apply(pd.to_numeric, errors="coerce").fillna(0.0).clip(lower=0.0)
        ...
        raw_file = os.path.join(self.root_output, "ko_raw.csv")
        processed_file = os.path.join(self.root_output, "ko_processed.csv")
        ...
        print(f"原始 KO 数据已保存: {raw_file}")
        print(f"预处理 KO 数据已保存: {processed_file}")
    ...
    def generate_report(self):
        ...
        report_file = os.path.join(self.root_output, "ko_preprocessing_report.txt")
        with open(report_file, "w", encoding="utf-8") as f:
            f.write("\n".join(report_lines))

        metadata_file = os.path.join(self.root_output, "ko_preprocessing_metadata.json")
...
class KOBiomarkerScreener:
    """KO biomarker screener aligned with the stronger species pipeline."""

    def __init__(
        self,
        preprocessed_metadata,
        root_output="/root/XXXMicro/Data/EW-T2_new/ko_biomarkers",
        top_k=200,
        candidate_pool_size=1000,
        eval_top_k=20,
        redundancy_threshold=0.95,
        export_filename="ko_abundance.csv",
    ):
    ...
    def select_biomarkers(self):
        print("\n" + "=" * 70)
        print(f"步骤1: KO 生物标志物筛选 (Top {self.top_k})")
        print("=" * 70)
        ...
        mw_df.to_csv(os.path.join(self.root_output, "ko_mann_whitney.csv"), index=False, encoding="utf-8")
        mw_sig_df.to_csv(
            os.path.join(self.root_output, "ko_mann_whitney_significant.csv"),
            index=False,
            encoding="utf-8",
        )
        rf_df.to_csv(os.path.join(self.root_output, "ko_rf_importance.csv"), index=False, encoding="utf-8")
        elastic_net_df.to_csv(
            os.path.join(self.root_output, "ko_elastic_net_coef.csv"),
            index=False,
            encoding="utf-8",
        )
        mi_df.to_csv(os.path.join(self.root_output, "ko_mutual_info.csv"), index=False, encoding="utf-8")
        integration_df.to_csv(os.path.join(self.root_output, "ko_integration_all.csv"), index=False, encoding="utf-8")
        skipped_df.to_csv(
            os.path.join(self.root_output, "ko_redundancy_skipped.csv"),
            index=False,
            encoding="utf-8",
        )
        final_df.to_csv(os.path.join(self.root_output, "ko_biomarkers.csv"), index=False, encoding="utf-8")
        with open(os.path.join(self.root_output, "ko_method_summary.json"), "w", encoding="utf-8") as f:
            json.dump(convert_numpy_types(self.method_summary), f, indent=2, ensure_ascii=False)

        with open(os.path.join(self.root_output, "ko_biomarker_list.txt"), "w", encoding="utf-8") as f:
            f.write("KO 层级生物标志物列表\n")
            f.write("=" * 50 + "\n\n")
    ...
    def evaluate_performance(self):
        ...
        with open(os.path.join(self.root_output, "ko_performance.json"), "w", encoding="utf-8") as f:
            json.dump(convert_numpy_types(self.performance), f, indent=2, ensure_ascii=False)

        return self.performance

    def generate_filtered_data(self):
        """Generate the final filtered KO abundance table."""
        print("\n" + "=" * 70)
    ...
    def generate_summary_report(self):
        """Generate the KO screening summary report."""
        report_lines = [
            "=" * 80,
            "KO 层级生物标志物筛选报告",
            "=" * 80,
        ...
        report_file = os.path.join(self.root_output, "ko_screening_report.txt")
        with open(report_file, "w", encoding="utf-8") as f:
            f.write("\n".join(report_lines))
...
def main():
    ...
    raw_data_file = "/root/XXXMicro/Data/EW-T2D-5-13/merged_ko.csv"
    preprocess_output = "/root/XXXMicro/Data/EW-T2D-5-13/ko_preprocessed"
    screening_output = "/root/XXXMicro/Data/EW-T2D-5-13/ko_biomarkers"

    if not os.path.exists(raw_data_file):
        print(f"错误: 原始数据文件不存在 - {raw_data_file}")
        return

    preprocessor = KOPreprocessor(
