...
def train(disease, feature, model_type, **params):
    ...
    if model_type == "FT_transformer":
        ...
        if ',' not in feature:
            raise ValueError("The MBT model requires multimodal input. Please set --feature to 'ko,species'")
        modality_order = feature.split(',')
        if set(modality_order) != {'ko', 'species'}:
            raise ValueError("The feature of MBT must be 'ko,species' or 'species,ko'")
