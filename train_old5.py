...
def train(disease, feature, model_type, **params):
    ...
    gat_heads = int(params.get('gat_heads', 4))
    gat_dropout = float(params.get('gat_dropout', 0.1))
    finetune_mbt = flag = params.get('finetune_mbt', False)
    ...
    if model_type == 'MBT':
        ...
        metric_cols = ['AUC', 'Recall', 'Precision', 'F1']
        ordered_cols = ['fold','lr','batch_size','feature','n_blocks','fusion_layer','num_bottleneck','proj_dim','gat_dim','gat_heads','gat_dropout','finetune_mbt','seed'] + metric_cols
