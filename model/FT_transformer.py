...
class FeatureTokenizer(nn.Module):
    ...
    def __init__(
        ...
        # One-time debug flag for printing token shapes
        self._printed_token_shapes = False
    ...
    def forward(self, x_num: Optional[Tensor], x_cat: Optional[Tensor]) -> Tensor:
        ...
        tokens = x[0] if len(x) == 1 else torch.cat(x, dim=1)
        if not self._printed_token_shapes:
            try:
                print(f"[FeatureTokenizer.debug] tokens shape: {list(tokens.shape)} (batch_size, n_tokens, d_token)")
            except Exception:
                pass
            self._printed_token_shapes = True
        return tokens
